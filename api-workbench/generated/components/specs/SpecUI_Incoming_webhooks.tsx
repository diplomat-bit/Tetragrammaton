...
export interface SpecUI_Incoming_webhooksProps {
...
const SPEC_META = {
  ...
  version: "1.0.0",
  format: "openapi_3",
  description: "It provides the capability to a partner for publishing event notifications from partner application to U.S. Bank",
  baseUrl: "https://api2.usbank.com/event-notifications/webhook/v1",
  endpoints: [{"id":"POST__events","path":"/events","method":"POST","summary":"Event notifications from Partners.","operationId":"publish-Events","parameters":[{"name":"Authorization","in":"header","required":true,"type":"string","description":"OAuth Bearer Token","example":""},{"name":"Correlation-ID","in":"header","required":true,"type":"string","description":"Unique identifier for every request-response pair. Allows to trace full path of interactions through multiple sub-systems.","example":""},{"name":"Content-Type","in":"header","required":true,"type":"string","description":"Indicates request payload format of 'application/json'.","example":"application/json"},{"name":"Accept","in":"header","required":true,"type":"string","description":"Indicates the acceptable response format. Recommended to use application/json.","example":""}],"hasBody":true,"samplePayload":{}}]
};

export const SpecUI_Incoming_webhooks: React.FC<SpecUI_Incoming_webhooksProps> = ({ onExecute }) => {
