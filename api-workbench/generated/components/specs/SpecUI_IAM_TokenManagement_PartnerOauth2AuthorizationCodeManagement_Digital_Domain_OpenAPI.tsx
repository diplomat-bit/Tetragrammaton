...
const SPEC_META = {
  ...
  version: "1.3.0",
  format: "openapi_3",
  description: "Authorize is our implementation of the OAuth 2.0 framework. It enables secure authorization using standard methods that can easily be integrated in your app. If you're familiar with OAuth 2.0, everything should look familiar. If not, you ma",
  baseUrl: "https://127.0.0.1/openapi/iam/tokenManagement/partner/authCode/oauth2/cgw/v1",
  endpoints: [{"id":"GET__authorize","path":"/authorize","method":"GET","summary":"Authorization code grant: Retrieve authorization code","operationId":"RetrieveAuthorizationCode","parameters":[{"name":"response_type","in":"query","required":true,"type":"string","description":"Value MUST be set to \"code\"","example":""},{"name":"client_id","in":"query","required":true,"type":"string","description":"Client ID generated during application registration.","example":""},{"name":"scope","in":"query","required":true,"type":"string","description":"The set of scopes required to make the API calls. Scope is case insensitive and multiple values can be passed using space delimiter.","example":""},{"name":"countryCode","in":"query","required":true,"type":"string","description":"Country code in 2 character ISO 3166 format (upper case)","example":""},{"name":"businessCode","in":"query","required":true,"type":"string","description":"The 3 character business code. Use GCB for consumer banking (upper case)","example":""},{"name":"locale","in":"query","required":true,"type":"string","description":"Locale identify a specific language and geographic region, it shoule follow [language[_territory]. eg - en_US, en_SG","example":""},{"name":"state","in":"query","required":true,"type":"string","description":"Opaque value to maintain the state between request and call back. This will be used to prevent cross-site request forgery.","example":""},{"name":"redirect_uri","in":"query","required":true,"type":"string","description":"Absolute uri for user-agent redirection.You should provide the uri used during client registration process.","example":""}],"hasBody":false,"samplePayload":{"referenceId":"REF-1789424098633","status":"PENDING","metadata":{"client":"API_WORKBENCH_SDK"}}},{"id":"POST__token__countryCode___businessCode_","path":"/token/{countryCode}/{businessCode}","method":"POST","summary":"Authorization code grant: Retrieve access token","operationId":"RetrieveAccessToken","parameters":[{"name":"Authorization","in":"header","required":true,"type":"string","description":"HTTP Basic authentication by passing base64 encoded value of the client id and client secret separated by colon (:).Example: Base64(client_id:client_secret) will be passed as Basic KGNsaWVudF9pZDpjbGllbnRfc2VjcmV0KQ==","example":""},{"name":"Content-Type","in":"header","required":true,"type":"string","description":"Value should always be application/x-www-form-urlencoded","example":""},{"name":"countryCode","in":"path","required":true,"type":"string","description":"countryCode","example":""},{"name":"businessCode","in":"path","required":true,"type":"string","description":"businessCode","example":""}],"hasBody":true,"samplePayload":{"grant_type":"sample_string","code":"sample_string","redirect_uri":"sample_string"}},{"id":"POST__refresh","path":"/refresh","method":"POST","summary":"Authorization code grant: Refresh access token","operationId":"RefreshToken","parameters":[{"name":"Authorization","in":"header","required":true,"type":"string","description":"HTTP Basic authentication by passing base64 encoded value of the client id and client secret separated by colon (:).Example: Base64(client_id:client_secret) will be passed as Basic KGNsaWVudF9pZDpjbGllbnRfc2VjcmV0KQ==","example":""},{"name":"Content-Type","in":"header","required":true,"type":"string","description":"Content type. Value is application/x-www-form-urlencoded","example":""}],"hasBody":true,"samplePayload":{"grant_type":"sample_string","refresh_token":"sample_string"}},{"id":"POST__revoke","path":"/revoke","method":"POST","summary":"Authorization code grant: Revoke access","operationId":"RevokeToken","parameters":[{"name":"Authorization","in":"header","required":true,"type":"string","description":"HTTP Basic authentication by passing base64 encoded value of the client id and client secret separated by colon (:).Example: Base64(client_id:client_secret) will be passed as Basic KGNsaWVudF9pZDpjbGllbnRfc2VjcmV0KQ==","example":""},{"name":"Content-Type","in":"header","required":true,"type":"string","description":"Content type. Value is application/x-www-form-urlencoded","example":""}],"hasBody":true,"samplePayload":{"token":"sample_string","token_type_hint":"sample_string"}}]
};

export const SpecUI_IAM_TokenManagement_PartnerOauth2AuthorizationCodeManagement_Digital_Domain_OpenAPI: React.FC<SpecUI_IAM_TokenManagement_PartnerOauth2AuthorizationCodeManagement_Digital_Domain_OpenAPIProps> = ({ onExecute }) => {
