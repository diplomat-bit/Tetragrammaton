...
// 1. ComplianceOracleView.tsx
let cov = fs.readFileSync('components/ComplianceOracleView.tsx', 'utf8');
if (!cov.includes('export default ComplianceOracleView')) {
  cov += '\nexport default ComplianceOracleView;\n';
  fs.writeFileSync('components/ComplianceOracleView.tsx', cov, 'utf8');
  console.log('Fixed ComplianceOracleView export');
}

// 2. ResourceGraphView.tsx
let rgv = fs.readFileSync('components/ResourceGraphView.tsx', 'utf8');
// rename second or duplicate getUserDashboard
let count = 0;
rgv = rgv.replace(/getUserDashboard\(\)/g, (match) => {
  count++;
  return count > 1 ? `getUserDashboardSnapshot()` : match;
});
fs.writeFileSync('components/ResourceGraphView.tsx', rgv, 'utf8');
console.log('Fixed ResourceGraphView duplicate members');

// 3. WebhookSimulator.tsx
const webhookSimulator = `import React, { useState } from 'react';
...
`;
fs.writeFileSync('components/WebhookSimulator.tsx', webhookSimulator, 'utf8');
console.log('Fixed WebhookSimulator.tsx');
