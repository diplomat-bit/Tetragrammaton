...
export async function fetchRepoTree(token: string, owner: string, repo: string, branch: string): Promise<(DirNode | FileNode)[]> {
  try {
    ...
    if (err.message && (err.message.includes('409') || err.message.includes('Git Repository is empty'))) {
      console.warn(`Repository ${owner}/${repo} is empty (409). Returning empty file tree.`);
      return [];
    }
    throw err;
  }
