...
export type BulkEditJobStatus = 'queued' | 'researching' | 'processing' | 'retrying' | 'success' | 'skipped' | 'failed';
...
export type ProjectExpansionJobStatus = 'queued' | 'researching' | 'generating' | 'committing' | 'retrying' | 'success' | 'failed';
...
export interface ReasoningMemoryStep {
    attempt: number;
    timestamp: string;
    model: string;
    agentRole: 'Architect' | 'Code Crafter' | 'CI Verifier' | 'Self-Correction Critic';
...
export interface AdvancedEditSessionState {
    repoFullName: string;
    branch: string;
    instruction: string;
    workflowId: string;
