...
const StatusIcon: React.FC<{ status: ProjectGenerationJob['status'] }> = ({ status }) => {
    switch (status) {
        ...
        case 'generating': 
            return <Spinner className="w-3.5 h-3.5 text-blue-400" />;
        case 'committing': 
      ...
      <div className="bg-gray-900 border border-gray-750 p-6 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col text-gray-200">
        <div className="flex justify-between items-center mb-4 flex-shrink-0">
            <div className="flex items-center gap-3">
              <h2 className="text-xl font-bold text-indigo-400">Massive Project Generation</h2>
              <span className="bg-indigo-950/80 text-indigo-300 border border-indigo-600 text-xs font-semibold px-2.5 py-0.5 rounded-full flex items-center gap-1.5">
            ...
            {isComplete && (
              <button
                onClick={onClose}
                className="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-semibold rounded-lg shadow-sm transition-colors"
              >
                Done
              </button>
            )}
        ...
        <div className="mb-5 flex-shrink-0 bg-gray-850 p-4 rounded-xl border border-gray-800">
            <div className="flex justify-between text-xs text-gray-300 mb-2">
                <span className="flex items-center gap-2 font-semibold">
                    {!isComplete && <Spinner className="h-3.5 w-3.5 text-indigo-400" />}
                    {statusMessage}
                </span>
            ...
            <div className="w-full bg-gray-800 rounded-full h-2 overflow-hidden">
                <div 
                  className="bg-gradient-to-r from-indigo-500 to-emerald-500 h-2 rounded-full transition-all duration-300 ease-out" 
            ...
            <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-gray-800/80 text-[11px]">
              <div className="flex items-center gap-2 text-gray-400">
                <span className="text-indigo-400 font-bold">⚡ Active Streams:</span>
                <span className="text-gray-200 font-mono">{activeJobs.length}</span>
              </div>
              <div className="flex items-center gap-2 text-gray-400">
                <span className="text-blue-400 font-bold">🔍 Pipeline:</span>
                <span className="text-gray-200">10-Stage Chained</span>
              </div>
              <div className="flex items-center gap-2 text-gray-400">
                <span className="text-emerald-400 font-bold">🛡️ Buffer GC:</span>
        ...
        <div className="flex-grow min-h-0 flex flex-col bg-gray-850 rounded-xl p-4 border border-gray-800 overflow-hidden">
             <div className="flex justify-between items-center mb-3">
               <h3 className="text-sm font-bold text-gray-200 uppercase tracking-wider">File Architecture Plan</h3>
               <span className="text-xs text-gray-400 font-mono">{jobs.length} Planned Files</span>
             </div>
                         ...
                         <div className="flex items-center gap-2 pl-3">
                           <span className={`px-2.5 py-0.5 rounded-full font-mono text-[10px] uppercase font-bold tracking-wider ${
                             job.status === 'success'
                               ...
                               : job.status === 'generating'
                               ? 'bg-indigo-900/80 text-indigo-300 border border-indigo-700 animate-pulse'
                               : job.status === 'committing'
        ...
        <div className="mt-4 flex justify-between items-center text-xs text-gray-400 flex-shrink-0">
          <span>⚡ Generating full multi-stage file contents silently in background without DOM memory bloat.</span>
          {isComplete && (
            <button
              onClick={onClose}
