...
const lazyComponentsCache: Record<string, React.LazyExoticComponent<React.ComponentType<any>>> = {};
...
const getComponentForView = (view: string) => {
    ...
    for (const path in modules) {
        ...
        if (possibleFileNames.includes(lowerFileName)) {
            if (!lazyComponentsCache[path]) {
                lazyComponentsCache[path] = React.lazy(modules[path] as any);
            }
            Component = lazyComponentsCache[path];
            break;
        }
    }
        ...
        return (
            <React.Suspense fallback={
                <div className="flex items-center justify-center h-64 text-cyan-400">
                    <Cpu className="w-8 h-8 animate-spin mr-3" />
